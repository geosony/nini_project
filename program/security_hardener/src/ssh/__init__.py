...
from security_hardener.src.ssh import actions

def harden():

    pl = parse_template(ctx=context_config["ssh"])
    pc = parse_compiler(ctx=context_config["ssh"])
    ps = parse_template(ctx=context_config["ssh"], parse_file="system")
